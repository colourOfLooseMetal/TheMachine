...
manifest.json           — SINGLE SOURCE OF TRUTH for show order, per-show line
                          counts, uncompressed byte sizes, and image dirs. A row may
                          carry an optional `base` (image-host root); rows without it
                          use the legacy colourofloosemetal.com/assets/ host, pipeline
                          rows point at the DigitalOcean Space. Generated by
                          pythonUtilityStuff/getKeyFromJson.py or
